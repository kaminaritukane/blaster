...
void UCombatComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ThisClass, HoldingFlag);
}
...
void UCombatComponent::FireProjectileWeapon()
{
	if (EquippedWeapon && Character)
	{
		HitTarget = EquippedWeapon->bUseScatter
			? EquippedWeapon->TraceEndWithScatter(HitTarget)
			: HitTarget;
...
void UCombatComponent::EquipWeapon(AWeapon* WeaponToEquip)
{
	...
	else
	{
		...
		{
			EquipSedondaryWeapon(WeaponToEquip);
		}
		else
		{
...
void UCombatComponent::EquipSedondaryWeapon(AWeapon* WeaponToEquip)
{
	if (WeaponToEquip == nullptr) return;
...
void UCombatComponent::AttachActorToLeftHand(AActor* ActorToAttach)
{
	...
	const USkeletalMeshSocket* HandSocket = Character->GetMesh()->GetSocketByName(
		bUsePistolSocket ? FName("PistolSocket") : FName("LeftHandSocket"));
	if (HandSocket)
	{
		HandSocket->AttachActor(ActorToAttach, Character->GetMesh());
	}
...
{
	//UE_LOG(LogTemp, Warning, TEXT("AttachFlagToLeftHand %s, Auth: %d"), 
	//	*Flag->GetFName().ToString(), 
	...
	//UE_LOG(LogTemp, Warning, TEXT("FlagSocket %d, CharaName: %s, Ctrlr: %s, Auth: %d"), 
	//	HandSocket, 
	...
	if (HandSocket)
	{
		bool bSucceeded = HandSocket->AttachActor(Flag, Character->GetMesh());
		//UE_LOG(LogTemp, Warning, TEXT("bSucceeded %d"), bSucceeded);
	}
}
...
{
	if (HoldingFlag && Character/* && !Character->HasAuthority()*/)
	{
		Character->Crouch();
		AttachFlagToLeftHand(HoldingFlag);
	}
}
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			HUDPackage.CrosshairVelocitySpread = CrosshairVelocityFactor + CrosshairInAirFactor;

			CrosshairShootingFactor = FMath::FInterpTo(CrosshairShootingFactor, 0.f, DeltaTime, 3.f);
			HUDPackage.CrosshairShootingFactor = CrosshairShootingFactor;

			HUDPackage.bIsCrouching = Character->bIsCrouched;
			HUDPackage.bIsAiming = bAiming;
...
void UCombatComponent::SetAiming(bool bIsAiming)
{
	...
	Character->GetCharacterMovement()->MaxWalkSpeedCrouched = bAiming ? AimWalkSpeedCrouched : BaseWalkSpeedCrouched;

	if (Character->IsLocallyControlled() && EquippedWeapon->GetWeaponType() == EWeaponType::EWT_SniperRifle)
	{
		Character->ShowSniperScopeWiget(bIsAiming);
	}
