...
FShotgunerverSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	UWorld* World = GetWorld();
	if (World == nullptr) return FShotgunerverSideRewindResult();

	for (auto& Frame : FramePackages)
	{
		if (Frame.Character == nullptr) return FShotgunerverSideRewindResult();
	}

	FShotgunerverSideRewindResult ShotgunResult;
	...
	// enable collision for all boxex, then disable for head boxes
	for (auto& Frame : FramePackages)
	{
...
FShotgunerverSideRewindResult ULagCompensationComponent::ShotgunServerSideRewind(const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
...
void ULagCompensationComponent::ServerScoreRequest_Implementation(ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime)
{
	...
	if (HitCharacter && Character->GetEquippedWeapon() && Character && Confirm.bHitConfirmed)
	{
...
{
	FShotgunerverSideRewindResult Confirm = ShotgunServerSideRewind(HitCharacters, TraceStart, HitLocations, HitTime);
	
	for (auto& HitCharacter : HitCharacters)
	{
		if (HitCharacter == nullptr 
			|| Character == nullptr
			|| Character->GetEquippedWeapon() == nullptr)
			continue;
